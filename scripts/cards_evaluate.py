...
NOTE:             
We need to keep rosbag, apriltag_ros and dope/feature_based_pose_est running
...
class CardsEvaluate(object):
    ...
    def tag_det_cb(self, msg):
        ...
        if dets:    
            ...
            self.tag_quat = np.mean(tag_quats, 0)
            # Transform tag's coordinate system to card's (180deg roll flip)
            self.tag_quat = self.quat_roll_flip(self.tag_quat)

        # If yaw angle is not within [-90, 90] deg, map the angle to within it   
        rpy = Rot.from_quat(self.tag_quat).as_euler("xyz", degrees=True)    
        if rpy[-1] > 90 or rpy[-1] < -90:
            self.tag_quat = self.quat_yaw_flip(self.tag_quat)
        # Uncomment to debug
    ...
    def dope_det_cb(self, msg):
        '''
        Callback function for /... msg (dope published)  
        Input: 
            msg(... msg)
        '''
        # TODO: implement 
